...
const RiderInboxScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    // Collect chat threads from riderDeliveries for this rider
    // This includes both regular rider deliveries and pasabuyer orders
    const riderDeliveriesRef = ref(db, `riderDeliveries/${userId}`);
    
    const unsub = onValue(riderDeliveriesRef, async (snap) => {
      if (!snap.exists()) { 
        setThreads([]); 
        return; 
      }
      ...
      for (const orderId of Object.keys(data)) {
        ...
        // Only include if chat exists and rider is a participant
    ...
    return () => off(riderDeliveriesRef, 'value', unsub);
  }, [userId]);
...
export default RiderInboxScreen;
